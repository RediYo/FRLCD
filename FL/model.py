...
def _training_loop(
    ...
    for features, labels in trainloader:
        features, labels = torch.tensor(features).to(device), torch.tensor(labels).to(device)
        optimizer.zero_grad()
        loss = criterion(net(features), torch.tensor(labels))
        loss.backward()
        optimizer.step()
    return net
...
def test(
    ...
    with torch.no_grad():
        for features, labels in testloader:
            outputs = net(torch.tensor(features))
            loss += criterion(outputs, torch.tensor(labels)).item()
            pred_y = outputs.numpy()
            label_y = np.array(labels)
            diff = abs(np.array(pred_y - label_y))
            if np.max(diff) <= 0.1:  # 输出概率差值全部小于0.1则认为是预测正确
                correct += 1
            total += 1
    if len(testloader.dataset) == 0:
        raise ValueError("Testloader can't be 0, exiting...")
    loss /= len(testloader.dataset)  # 平均损失
