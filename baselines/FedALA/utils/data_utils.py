...
trainloaders, valloaders, testloader = load_datasets(num_clients=10, partition=1, batch_size=32)
...
# RediYo
def read_client_data_partition(dataset, idx, is_train=True):

    if is_train:
        return trainloaders[idx]
    else:
        return testloader
