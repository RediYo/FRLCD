...
class ALA:
    ...
    def adaptive_local_aggregation(self, 
        ...
        # 获得数据集大小和需要获取的数据下标
        # print(f"len(train_data.dataset):{len(self.train_data.dataset)}")
        rand_ratio = self.rand_percent / 100
        rand_num = int(rand_ratio * len(self.train_data.dataset))
        rand_idx = random.randint(0, len(self.train_data.dataset) - rand_num)
        # 直接切片采样 rand_num 个样本
        subset = torch.utils.data.Subset(self.train_data.dataset, range(rand_idx, rand_idx + rand_num))
        # 构造仅包含 rand_num 个采样样本的 DataLoader
        rand_loader = DataLoader(subset, batch_size=16, shuffle=True)
        print(f"len(rand_loader.dataset):{len(rand_loader.dataset)}")
        # rand_ratio = self.rand_percent / 100
        ...
        while True:
            for x, y in rand_loader:
                ...
                loss_value = self.loss(output_tensor, y)  # modify according to the local objective
                loss_value.backward()
