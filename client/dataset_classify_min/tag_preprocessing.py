...
input_path = r".\2\submit\活动室"
input_path_2 = r".\2\活动室\3"
output_path = r".\data"
    ...
    for filename in filenames:
        for file_2 in in_p_2.glob('*.csv'):
            ...
            if (file_2.name.find(filename) != -1) or (file_2.name.find(filename_2) != -1):
                ...
                with open(file_2, 'a', encoding='GBK') as output:  # 在全局工作路径下
                    writer = csv.writer(output)  # 用writer函数读入文件指针
                    writer.writerow(dis_list[int(start_min):int(end_min+1)])
                    output.close()
