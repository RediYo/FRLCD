...
def get_ble_data(data_path):
    ...
    print(f"data:{(np.array(data)).reshape((np.array(data)).shape[0], -1).astype(np.float32)}")
    print(f"target:{np.array(target[0:len(data)])}")

    return np.array(data, dtype=object).reshape(np.array(data).shape[0], -1).astype(np.float32), np.array(target[0:len(data)], dtype=object)
