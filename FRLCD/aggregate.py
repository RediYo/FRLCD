...
def aggregate(results: List[Tuple[NDArrays, int]]) -> NDArrays:
    ...
    w_list = [w for _, w in results]
    w_total = sum(w_list)
    # 如何权重选择全为0则随机抽取一个客户端的w使其为1
    if all(w == 0 for w in w_list):
        w_list[random.randint(0, len(w_list))] = 1
        w_total += 1
        weighted_weights = [
            [layer * w for layer in res[0]] for w, res in zip(w_list, results)
        ]
    else:
        weighted_weights = [
            [layer * w for layer in weights] for weights, w in results
        ]

    # Compute average weights of each layer
    weights_prime: NDArrays = [
        reduce(np.add, layer_updates) / w_total
...
def weighted_accuracy_avg(client_num: int, results: List[float]) -> float:
    """Aggregate evaluation results obtained from multiple clients."""
    weighted_accuracy = [accuracy for accuracy in results]
    return sum(weighted_accuracy) / client_num
